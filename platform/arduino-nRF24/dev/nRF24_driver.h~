...
s   *
   ...
   */
  bool nRF24_writeBlocking( const void* buf, uint8_t len, uint32_t timeout );
   ...
   *
   */
   bool nRF24_txStandBy_def(uint32_t timeout, bool startTx);
   ...
   * by the payload set by setPayloadSize().
   */
   ...
   * @return True if the payload was delivered successfully false if not
   */
  void nRF24_startFastWrite( const void* buf, uint8_t len, const bool multicast, bool startTx);
